...
export const TradeHistory = ({ historyList, setHistoryList }: TradeHistoryProps) => {
  ...
  const getTradeHistory = useCallback(async () => {
    try {
      const quantosRequest = await fetch(QUANTO_API + '?chainId=97&offset=0&limit=' + allPoolParams.length)
      ...
      if (quantos.code == 200) {
        ...
        const historyRequest = await fetch(
          TRADE_HISTORY_API + `?chainId=97&trader=${account}&indexId=${target.id}&offset=0&limit=100`
        )
