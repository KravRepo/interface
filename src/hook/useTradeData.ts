...
export function useTradeData({
  ...
  useEffect(() => {
    const fetchLiquidationPrice = async () => {
      ...
      if (pairContract && liquidationPriceArgs.every(arg => arg !== undefined) && positionSizeDai?.div(leverage).times(1e18).toString().split('.')[0] !== '0') {
        try {

          const result = await retryAsync(pairContract.getTradeLiquidationPrice, liquidationPriceArgs);
          setLiquidationPrice('$' + (result / 10 ** 10).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        } catch (error) {
    ...
    const fetchPriceImpact = async () => {
      if (pairContract && priceImpactArgs.every(arg => arg !== undefined)) {
        try {
          ...
          setPriceImpact('$' + (result.priceAfterImpact / 10 ** 10).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        } catch (error) {
