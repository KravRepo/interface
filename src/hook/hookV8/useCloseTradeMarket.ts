    ...
    async (orderIndex: number, pairIndex = 0) => {
      try {
        ...
        let gasLimit = await getGasLimit(
          ...
          'closeTradeMarket',
          params,
          new BigNumber(minETHFees._hex).toString()
        )
        gasLimit = new BigNumber(gasLimit.toString()).times(1.1)
        setTransactionState(TransactionState.INTERACTION)
        setTransactionDialogVisibility(true)
        const tx = await contract.closeTradeMarket(...params, {
  ...
  return useCallback(
    async (isSL: boolean, price: BigNumber, orderIndex: number, pairIndex = 0) => {
      try {
        ...
        const minETHFees = await contract.minExecutionFee()
        let gasLimit = await getGasLimit(contract, func, params, new BigNumber(minETHFees._hex).toString())
        gasLimit = new BigNumber(gasLimit.toString()).times(1.1)
        setTransactionState(TransactionState.INTERACTION)
        setTransactionDialogVisibility(true)
        const tx = await contract[func](...params, {
          value: new BigNumber(minETHFees._hex).toString(),
          gasLimit: gasLimit.toFixed(0),
        })
